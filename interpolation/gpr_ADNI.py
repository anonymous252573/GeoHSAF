...
def load_pkl_data(folder, time_map):
    ...
    subject_id_map = {}     # Map string IDs to integer IDs
    current_id = 1
    ...
    return np.array(all_X), np.vstack(all_Y), subject_data, subject_id_map   #subject data (ID and the PGA vector across time); subject_id_map (encode the ID)


condis = ['AD', 'CN', 'MCI']
for condi in condis:

    data_folder = r'C:\Users\......\ADNI\After_matching\{}_PGA_tangent_data'.format(condi) 
    save_folder = r'C:\Users\.......\ADNI\After_matching\{}_interpolated_PGA_tangent_data'.format(condi)
    os.makedirs(save_folder, exist_ok=True) 

    # Dynamically build time map
    time_map = extract_time_map(data_folder)  # e.g., {'bl': 1, 'm03': 3, 'm06': 6, 'm12': 12, 'm18': 18, 'm24': 24, 'm30': 30, 'm36': 36, 'm60': 60, 'm78': 78, 'm96': 96}
    # Load all data using the time map
    ...
    # X_tensor = X_tensor.prod(dim=1)    # Y are the PGA vectors and X are the time points with subject ID encoded
