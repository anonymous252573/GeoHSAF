...
# ***** We load the data for each subject ****
def load_subject_time_series(folder_path, time_map):
    ...
    for filename in os.listdir(folder_path):
        ...
        try:
            subject_id = filename.lower().split('_bl')[0]
        except IndexError:
...
subject_AD_dict = load_subject_time_series(data_folder_AD, time_map_AD)  
subject_CN_dict = load_subject_time_series(data_folder_CN, time_map_CN)
...
def get_first_array_dimension(d):
    for subj in d:
        for t in d[subj]:
            return d[subj][t].shape[-1]  # assuming 2D or 1D
