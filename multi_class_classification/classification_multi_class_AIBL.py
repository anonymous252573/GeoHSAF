...
data_folder_AD = r'C:\....\AIBL\After_matching\{}_PCA_tangent_data'.format(condi_AD) 
data_folder_CN = r'C:\....\AIBL\After_matching\{}_PCA_tangent_data'.format(condi_CN) 
data_folder_MCI = r'C:\....\AIBL\After_matching\{}_PCA_tangent_data'.format(condi_MCI) 

data_folder_interp_AD = r'C:\....\AIBL\After_matching\{}_interpolated_PCA_tangent_data'.format(condi_AD) 
data_folder_interp_CN = r'C:\....\AIBL\After_matching\{}_interpolated_PCA_tangent_data'.format(condi_CN) 
data_folder_interp_MCI = r'C:\....\AIBL\After_matching\{}_interpolated_PCA_tangent_data'.format(condi_MCI) 
...
# **** Let's collect all available timepoints in a dictionary for all groups ****
time_map_AD = extract_time_map(data_folder_AD)
time_map_CN = extract_time_map(data_folder_CN)
time_map_MCI = extract_time_map(data_folder_MCI)
new_entries = {'m09': 9, 'm14': 14, 'm27': 27, 'm45': 45, 'm63': 63}
time_map_AD.update(new_entries)
time_map_CN.update(new_entries)
time_map_MCI.update(new_entries)
time_map_AD = dict(sorted(time_map_AD.items(), key=lambda item: item[1]))
time_map_CN = dict(sorted(time_map_CN.items(), key=lambda item: item[1]))
time_map_MCI = dict(sorted(time_map_MCI.items(), key=lambda item: item[1]))
...
subject_AD_dict = load_interpolated_subject_time_series(data_folder_interp_AD, time_map_AD)
subject_CN_dict = load_interpolated_subject_time_series(data_folder_interp_CN, time_map_CN)
subject_MCI_dict = load_interpolated_subject_time_series(data_folder_interp_MCI, time_map_MCI)  

# **** Small ablation here (interested to see the performance by removing those with only baseline scan from the MCI subjects) - start ****
# otherwise, comment this block ****
subject_MCI_dict = {
    ID: time_data
    for ID, time_data in subject_MCI_dict.items()
    if len(time_data) > 1
}
# print (len(subject_MCI_dict.keys())) - 30 subjects !!
# ****** End ******
...
dim3 = get_first_array_dimension(subject_MCI_dict)
...
if dim3 < max_dim:
    subject_MCI_dict = pad_dict_arrays(subject_MCI_dict, max_dim)
...
MCI_data = prepare_subject_data(subject_MCI_dict, label=2)
all_data = CN_data + AD_data + MCI_data
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim, model_dim, num_classes, num_heads=4, num_layers=4, dropout=0.1):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, model_dim))  # learnable [CLS] token
...
input_dim = train_data[0][0][0].shape[0]  # get the size of the PCA vectors
model = TransformerClassifier(input_dim=input_dim, model_dim=256, num_classes=3).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
criterion = nn.CrossEntropyLoss()

def train_one_epoch(model, dataloader, optimizer, criterion, num_classes=3):
    model.train()
    ...
    for x, y, mask in dataloader:
        ...
        # this is for AUC (multi-class probabilities)
...
@torch.no_grad()
def evaluate(model, dataloader, criterion, num_classes=3):
    model.eval()
    total_loss, correct, total = 0, 0, 0
    all_labels = []
    all_probs = []
